...
durbin_watson_test.lm <- function(object,
                                  ...
                                  ...,
                                  .alpha = 0.05) {
  durbin_watson_test_impl(object, alternative, ..., .alpha = .alpha)
}
...
durbin_watson_test._lm <- function(object,
                                   ...
                                   ...,
                                  .alpha = 0.05) {
  durbin_watson_test_impl(object[["fit"]], alternative, ..., .alpha = .alpha)
}
...
ljung_box_test.lm <- function(object, ..., .alpha = 0.05) {
  ljung_box_test_impl(calc_residuals(object), ..., .alpha = .alpha)
}

#' @rdname ljung_box_test
#' @export
ljung_box_test._lm <- function(object, ..., .alpha = 0.05) {
  ljung_box_test_impl(calc_residuals(object[["fit"]]), ..., .alpha = .alpha)
}
...
ljung_box_test_vec <- function(x, ..., .alpha = 0.05) {
  ljung_box_test_impl(x, ..., .alpha = .alpha)
}


# Helper Functions ------------------------------------------------------------
durbin_watson_test_impl <- function(object,
                                    alternative = "two.sided",
                                    ...,
                                    .alpha = 0.05) {
  tidy_test(
...
ljung_box_test_impl <- function(resids, ..., .alpha = 0.05) {
  tidy_test(
    resids,
    Box.test,
    type = "Ljung-Box",
